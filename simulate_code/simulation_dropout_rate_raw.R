setwd("/data2/users/zengys/data/singlecell/number_class/splatter_simulation/no_normalized_data")
...
for(i in 1:20) {
  ...
  batch_1_data <- rbind(sample_labels = rep(1, length(Y)), cell_labels = Y, cluster_labels= rep(1, length(Y)), X)
   write.table(batch_1_data, file = paste("splatter_simulate_",i,"_" ,facScale,".csv",sep=""), sep = ",", quote = F, col.names = F, row.names = T)
